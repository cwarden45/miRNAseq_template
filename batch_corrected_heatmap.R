...
deg.genes = deg.table$miRNA[deg.table$status != "No Change"]
...
expression.genes = as.character(expression.table$miRNA)
	
temp.cpm = expression.table[match(deg.genes, expression.genes, nomatch=0),]
temp.cpm = temp.cpm[,match(sample.label, names(temp.cpm))]
temp.cpm = log2(temp.cpm+1)
print(dim(temp.cpm))
temp.cpm = t(apply(temp.cpm, 1, batch.center, batchID = batchID))
print(dim(temp.cpm))
