...
#### 6) Calculate Count-Per-Million (CPM) abundance.  Templates provided here include `calculate_htseq_cpm.R` and `calculate_featureCounts_cpm.R`.  Downstream analysis can work with any CPM table formatted with the miRNA in the first column and the short sample ID in the remaining columns.
