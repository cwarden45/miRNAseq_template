...
#### 5) Quantify read counts.  You can use your read counting method of choice, as long as the format matches input for the CPM function (miRNA in 1st column, final sample names for remaining columns).  Alternatively, you can use `cluster_HTseq_counts.py` and `HTseq_reformat.R` (or `cluster_featureCounts.py` and `featureCounts_reformat.R`).

These scripts are slightly different than those used in [TopHat_Workflow](https://github.com/cwarden45/RNAseq_templates/tree/master/TopHat_Workflow).  More specifically, only counts are tabulated (without RPKM values) and two types of counts are provided: mature miRNA and "other" genes.  For miRNAs, HT-Seq is switched to "intersection-strict".  The script `combine_gtf.py` can create the "other" gene table as a mix of UCSC RepeatMasker rRNA (with download instructions in comments), UCSC tRNA, and the gene annotations from [Genome_Ref_Code](https://github.com/cwarden45/RNAseq_templates/tree/master/Genome_Ref_Code) (includes primary miRNA and snoRNAs, for example).

#### 6) Calculate Count-Per-Million (CPM) abundance
